CreateIniStates <- function(
  ...
  FeatFUN_MOD <- attr(InputsModel, "FeatFUN_MOD")
  if (
    is.null(FeatFUN_MOD) ||
      !identical(FUN_MOD, match.fun(FeatFUN_MOD$NameFunMod))
  ) {
    warning("'FUN_MOD' does not match the model used in 'InputsModel'")
  ...
  if (
    ...
    if (verbose) {
      warning(sprintf(
        "'%s' does not require 'ExpStore'. Value set to NA",
  ...
  if (
    ...
    if (verbose) {
      warning(sprintf(
        "'%s' does not require 'IntStore'. Values set to NA",
        FeatFUN_MOD$NameFunMod
      ))
    }
    IntStore <- Inf
  ...
  if (
    "CemaNeige" %in%
      ...
      (is.null(GCemaNeigeLayers) |
        ...
        is.null(GlocmaxCemaNeigeLayers))
  ) {
    stop(sprintf(
  ...
  if (
    ...
    stop(
      "Negative values are not allowed for any of 'ProdStore', 'RoutStore', 'IntStore', 'UH1', 'UH2', 'GCemaNeigeLayers'"
    )
  }
  ...
  if (
    "hourly" %in% ObjectClass & (!is.numeric(UH1) || length(UH1) != UH1n * 24)
  ) {
  ...
  if (
    "hourly" %in% ObjectClass & (!is.numeric(UH2) || length(UH2) != UH2n * 24)
  ) {
  ...
  if (
    !is.numeric(eTGCemaNeigeLayers) || length(eTGCemaNeigeLayers) != NLayers
  ) {
    stop(sprintf("'eTGCemaNeigeLayers' must be numeric of length %i", NLayers))
  }
  if (IsHyst) {
    if (
      !is.numeric(GthrCemaNeigeLayers) || length(GthrCemaNeigeLayers) != NLayers
    ) {
      stop(sprintf(
    ...
    if (
      !is.numeric(GlocmaxCemaNeigeLayers) ||
        length(GlocmaxCemaNeigeLayers) != NLayers
    ) {
      stop(sprintf(
        "'eTGCemaNeigeLayers' must be numeric of length %i",
        NLayers
  ...
  if (!is.null(SD)) {
    ...
    lapply(SD, function(x) {
      if (!is.numeric(x)) {
        stop("Each item of 'SD' list argument must be numeric")
      }
    })
    if (length(SD) != length(InputsModel$LengthHydro)) {
      stop(
        ...
        sprintf(
          " (%i required, found %i)",
          length(InputsModel$LengthHydro),
          length(SD)
        )
      )
    }
  ...
  IniStates <- list(
    ...
    CemaNeigeLayers = list(
      ...
      Gthr = GthrCemaNeigeLayers,
      Glocmax = GlocmaxCemaNeigeLayers
    )
  )
