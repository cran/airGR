...
Qupstream <- 0.5 +
  floor(
    (sin((seq_along(BasinObs$Qmm) / 365 * 2 * 3.14)) + 1) *
      mean(BasinObs$Qmm, na.rm = TRUE)
  )
...
test_that("CreateRunOptions runs with RunModel_Lag", {
  ...
  expect_s3_class(RunOptions, "RunOptions")
  expect_s3_class(RunOptions, "SD")
})
...
test_that("RunModel(FUN=RunModel_Lag) should give same result as RunModel_Lag", {
  QcontribDown <- OutputsGR4JOnly
  Output_RunModel_Lag <- RunModel_Lag(
    InputsModel = InputsModel,
    RunOptions = RunOptions,
    Param = 1,
    QcontribDown = QcontribDown
  )
  Output_RunModel <- RunModel(
    InputsModel = InputsModel,
    RunOptions = RunOptions,
    Param = 1,
    FUN_MOD = RunModel_Lag,
...
test_that("1 input with lag of 0.5 time step delay out gives an output delayed of 0.5 time step", {
  ...
  Qm3SdSim <- OutputsSD$Qsim_m3
  Qm3UpstLagObs <- (Qupstream[Ind_Run] + Qupstream[Ind_Run - 1]) /
    2 *
    InputsModel$BasinAreas[1] *
    1e3
  expect_equal(Qm3SdSim - Qm3GR4Only, Qm3UpstLagObs)
})

test_that("Qupstream in different units should return the same result", {
  OutputsSD_mm <- RunModel(
    InputsModel,
    RunOptions,
    Param = ParamSD,
    FUN_MOD = RunModel_GR4J
...
test_that("Params from calibration with simulated data should be similar to initial params", {
  ...
  expect_equal(
    OutputsCalib$ParamFinalR[2:5] / ParamSD[2:5],
    rep(1, 4),
    tolerance = 1e-2
  )
...
test_that("OutputsModel$Qsim must don't have negative flows in Calibration", {
  InputsModel <- CreateInputsModel(
    FUN_MOD = RunModel_GR4J,
    DatesR = BasinObs$DatesR,
    Precip = BasinObs$P,
  ...
  InputsCrit <- CreateInputsCrit(
    ...
    VarObs = "Q",
    transfo = "sqrt",
    Obs = (sapply(-Qupstream[Ind_Run - 1], max, 0) *
      BasinAreas[1L] +
      BasinObs$Qmm[Ind_Run] * BasinAreas[2L]) /
      sum(BasinAreas)
  )
